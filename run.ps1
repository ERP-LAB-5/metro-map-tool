...
# ------------------------------------------------------------------ restart --
Stop-Server
Write-Host "  starting metro-map on http://${Bind}:$Port"
$appArgs = @("-m", "metro_map_tool.app", "--host", $Bind, "--port", $Port) + $Rest
...
if (-not $ready) {
    throw "metro-map did not come up on port $Port (pid $($proc.Id))"
}
