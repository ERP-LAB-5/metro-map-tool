...
| `continues` | On a line: `start`, `end` or `both` draws an arrowhead past that terminal stop, saying the route runs on beyond the map. The opposite of a `dead_end`, and drawn in the same place — never put both on one stop. |
