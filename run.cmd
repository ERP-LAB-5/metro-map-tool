...
:: Start metro-map on Windows, without needing PowerShell.
::
...
call :stop_server
if errorlevel 1 exit /b 1
echo   starting metro-map on http://%BIND%:%PORT%
:: its own minimised window, so the server outlives this script when run.cmd
:: was double-clicked and its console closes
start "metro-map" /min "%PY_VENV%" -m metro_map_tool.app --host %BIND% --port %PORT% %PASS%
...
if "%READY%"=="0" (
    echo   ! metro-map did not come up on port %PORT%
    exit /b 1
)
